...
import { addBasic, addServiceTag, addServiceTool, getAllServices, getCategories, getServiceWTagById, getServiceWToolById, getTools, updateServiceWithCategories } from '../_serviceActions/ServiceActions';
...
import { createServiceTag, createServiceTool, getTags, removeServiceTag, removeServiceTool } from '../../common/_actions/Actions';
...
const CodeCreate = ({serviceId}:Props) => {
...
const getSelected= (selected:string)=>{
   ...
   const addCode = async () => {
    try {
      ...
      setLoading(true);
      const names = await addServiceTag(serviceId, numberArray);
      setLoading(false);
      setCodeNames(names);
  ...
  const getCodeNames = async ()=> {
    try {
        setLoading(true);
        const names = await getServiceWTagById(serviceId);
        setLoading(false);
        setCodeNames(names);
        setError(null); 
  ...
  const removeCode  = async () => {
    ...
    try {
      setLoading(true);
      const tools = await removeServiceTag(serviceId,removedTool);
      setLoading(false);
     setCodeNames(tools);
     setError(null);
     setShowRemove(false);
    ...
    const createCode = async () => {
      try {
        if(serviceId != 0 && codeName != ''){
          setLoading(true);
          const cdname = await createServiceTag(serviceId , codeName);
          setLoading(false);
          setError(null)
          setCodeNames((prevToolNames) => [...prevToolNames, cdname]);
        }
        setShowAdd(false);
      } catch (error:any) {
         setError(error); 
    ...
    <div className="text-start z-40  ">
       <div className=" mb-3 border border-gray-200 rounded-md">
            <div className="px-2 border-b flex gap-2 items-center border-b-gray-200 py-1.5 ">
                ...
                <span className="text-gray-600 font-medium text-md">Service Code added </span>
            </div>
          {codeNames.length>0 ?  (
            ...
            {codeNames.map(name => (
              <div className="flex items-center rounded-md pl-2 pr-1 bg-gray-100 border border-gray-200">
                  ...
                  <button 
                  onClick={()=> { setRemovedTool(name);setShowRemove(true) }}
          ...
          <span className="text-sm text-orange-600 px-4 py-2 inline-flex capitalize">No Service Code Added </span>
        )
        }
        </div>
      ...
      <div className="border-t my-1.5 flex items-center justify-center pt-3 border-t-gray-200 ">
        <div className="flex items-center border gap-x-3 py-1 border-gray-200  px-2 bg-gray-100 rounded-md">
          ...
          <span className="text-md inline-flex text-indigo-600">Slecet Service Code</span>
        </div>
      ...
      {showRemove && <div className='fixed top-0 left-0 z-30 bg-[#00000018]  flex items-center justify-center h-full w-full '>
          <div className="w-5/12 bg-white animate-modalEnter rounded-md shadow-lg p-4">
                 <div className="flex w-full border border-gray-300 bg-gray-200 rounded-md py-2.5 items-center px-3 border-b border-b-gray-300" style={{boxShadow:'0 6px 19px -13px #9f9494;'}}>
                      <div className="flex items-center">
                          ...
                          <span className="text-base font-semibold text-gray-600">Remove Service Code</span>
                      </div>
                      <div className="ml-auto">
                          <button type="button" onClick={() => setShowRemove(false)}  className="text-gray-800 close-icon bg-gray-200 hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-6 h-6 inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" >
                  ...
                  <div className=" flex flex-col z-0 w-full mt-2 mb-5 group pt-2">
                      <span className='text-red-500'>You are going to remove Service Code</span>
                  </div>
      ...
      {showAdd && <div className='fixed top-0 left-0 z-30 bg-[#00000018]  flex items-center justify-center h-full w-full '>
          <div className="w-5/12 bg-white animate-modalEnter rounded-md shadow-lg p-4">
                 <div className="flex w-full border border-gray-300 bg-gray-200 rounded-md py-2.5 items-center px-3 border-b border-b-gray-300" style={{boxShadow:'0 6px 19px -13px #9f9494;'}}>
                      <div className="flex items-center">
                              <span className=""><MdAssignmentAdd className='text-gray-600 text-2xl mr-2' /> </span>
                          <span className="text-base font-semibold text-gray-600">Add New Code</span>
                      </div>
                      <div className="ml-auto">
                          <button type="button" onClick={() => setShowAdd(false)}  className="text-gray-800 close-icon bg-gray-200 hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-6 h-6 inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white" >
                              <svg className="w-2.5 h-2.5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
                                  <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
                              </svg>
                              <span className="sr-only">Close modal</span>
                  ...
                  <div className=" flex flex-col z-0 w-full mt-2 mb-5 group pt-2">
                      ...
                      <div className="flex items-center w-full">
                        ...
                        <input  
                        onChange={(e) => addCodeName(e.target.value)}
