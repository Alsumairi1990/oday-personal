...
import { addServiceCategories, addServiceTags, getServices, removeServiceCategory, removeServiceTag } from "../../service/_serviceActions/ServiceActions";
...
const TagPanel = ({ service, colseModel }: FormEditProps) => {
  ...
  const addService = async () => {
    try {
        ...
        const result = await addServiceTags(Number(service.id), numberArray);
  ...
  const removeElem = async ()=> {
    try {
        setLoading(true);
        const numberArray = selectedMenuElementsId.map(value => Number(value));
        const result = await removeServiceTag(service.id,removedTool);
                   ...
                   {serviceData.tags && serviceData.tags.length>0 ?  (
                          ...
                          {serviceData.tags.map(element => (
                            <div className="flex items-center  rounded-md pl-2 pr-1 bg-white border border-gray-200">
                                ...
                                <button
                                  onClick={() => {
                                    if(serviceData.tags.length>0) setRemovedTool(element.tag.name);
                        ...
                        <div className="pb-1">
                        <span className="text-sm text-orange-600 px-4 inline-flex capitalize">No Tags  </span>
                    </div>                                   
                      )
                      }
                  </div>
                    ...
                    >
                      ...
                      {selectedMenuElements.length>0 ? (
                        ...
                        <div className="text-md inline-flex text-gray-600 font-medium capitalize">
                          <span className="px-1">Adding Tag</span>
                        </div>
                      )}
