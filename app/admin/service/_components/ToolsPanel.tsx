...
import { addServiceCategories, addServiceTags, addServiceTools, getServices, removeServiceCategory, removeServiceTag, removeServiceTool } from "../../service/_serviceActions/ServiceActions";
...
const ToolsPanel = ({ service, colseModel }: FormEditProps) => {
  ...
  const addService = async () => {
    try {
        ...
        const result = await addServiceTools(Number(service.id), numberArray);
  ...
  const removeElem = async ()=> {
    try {
        setLoading(true);
        const numberArray = selectedMenuElementsId.map(value => Number(value));
        const result = await removeServiceTool(service.id,removedTool);
          ...
          <div className="px-2 w-full animate-modalSlide ">
            <div className="flex  pt-1 pb-2 ">
              <div className="flex gap-x-2 items-center">
                <span className="">
                  <FaTools className="text-gray-700 text-xl" />{" "}
                </span>
                ...
                <div className="py-2 flex items-center border-b border-b-gray-300 flex-wrap overflow-y-auto">
                  <div className="flex items-center rounded-md w-full gap-2 max-h-16 flex-wrap overflow-y-auto">
                   {serviceData.tools && serviceData.tools.length>0 ?  (
                          ...
                          {serviceData.tools.map(element => (
                            <div className="flex items-center  rounded-md pl-2 pr-1 bg-white border border-gray-200">
                                {element.tool.name && <span className='text-sm  text-orange-600 py-1 mr-2 font-medium'>{element.tool.name}</span>}
                                <button
                                  onClick={() => {
                                    if(serviceData.tools.length>0) setRemovedTool(element.tool.name);
                        ...
                        <div className="pb-1">
                        <span className="text-sm text-orange-600 px-4 inline-flex capitalize">No Tools  </span>
                    </div>                                   
                      )
                      }
                  </div>
                    ...
                    >
                      ...
                      {selectedMenuElements.length>0 ? (
                        ...
                        <div className="text-md inline-flex text-gray-600 font-medium capitalize">
                          <span className="px-1">Adding Tools</span>
                        </div>
                      )}
