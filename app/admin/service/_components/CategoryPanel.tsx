...
import { addServiceCategories, getServices, removeServiceCategory } from "../../service/_serviceActions/ServiceActions";
...
const CategoryPanel = ({ service, colseModel }: FormEditProps) => {
  ...
  const addService = async () => {
    try {
        ...
        const result = await addServiceCategories(Number(service.id), numberArray);
  ...
  const removeElem = async ()=> {
    try {
        setLoading(true);
        const numberArray = selectedMenuElementsId.map(value => Number(value));
        const result = await removeServiceCategory(service.id,removedTool);
                ...
                <div className="py-2 flex items-center border-b border-b-gray-300 flex-wrap overflow-y-auto">
                  <div className="flex items-center rounded-md w-full gap-2 max-h-16 flex-wrap overflow-y-auto">
                   {serviceData.categories && serviceData.categories.length>0 ?  (
                          ...
                          {serviceData.categories.map(element => (
                            <div className="flex items-center  rounded-md pl-2 pr-1 bg-white border border-gray-200">
                                ...
                                <button
                                  onClick={() => {
                                    if(serviceData.categories.length>0) setRemovedTool(element.category.name);
