...
import { addCategory, addingCategory } from '../_actions/Actions';
...
const BasicInfo = () => {
  ...
  const transformToCategoryInput = (data: inputType): CategoryInput => {
    ...
    const saveUser: SubmitHandler<inputType> = async (data)=>{
      ...
      try {
        const categ =  await addingCategory(formData);
        setCategory(categ);
      } catch (error) {
      }
    }
          ...
          <div className="px-2 py-2.5 w-full flex items-center rounded-md bg-[#7648e5] mb-3">
            ...
            <div className=" flex flex-col z-0 w-full mt-2 mb-5 group">
                    ...
                    <div className="flex items-center w-full">
                        ...
                        <input {...register('category_name')}  type="text" name="category_name" id="category_name" className="block pl-2 h-10 px-0 z-0 w-full text-sm text-gray-900 bg-transparent border rounded-xl border-gray-300 appearance-none   focus:outline-none focus:ring-0 focus:border-orange-500 peer" placeholder="Web Designing ..." required />
                        </div>
                    </div> 
            ...
            <div className=" flex flex-col z-0 w-full mb-5 group">
                    ...
                    <div className="flex items-center w-full">
                        <div className="relative flex w-full">
                        <input {...register('description')}  type="text" name="description" id="description" className="block pl-2 h-10 px-0 z-0 w-full text-sm text-gray-900 bg-transparent border rounded-xl border-gray-300 appearance-none   focus:outline-none focus:ring-0 focus:border-orange-500 peer" placeholder="Web Designing ..." required />
                        </div>
                    </div> 
            ...
            <div className="flex items-center mb-4 justify-center w-full">
                  <label htmlFor="icon" className="flex flex-col items-center justify-center w-full h-44 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50  hover:bg-gray-100">
                      ...
                      />
                        ...
                        <p className="mb-2 text-sm text-gray-500 "><span className="font-semibold">Upload Icon</span> From system</p>
                        <p className="text-xs text-gray-500 ">SVG, PNG, JPG or GIF (MAX. 800x400px)</p>
                        </div>
                  )} 
                      </div>
