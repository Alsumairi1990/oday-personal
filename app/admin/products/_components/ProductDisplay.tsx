...
const ProductDisplay = () => {
   ...
   const getAlproducts = async () => {
    try {
        setLoading(true)
        const catgs = await getProductWithModels();
        setproducts(catgs);
        setError('');
    } catch (error:any) {
        setLoading(false);
...
const closeConfirm = (flag:boolean) =>{
  ...
  return (
    <div className="relative sm:w-full sm:pb-8 mx-auto flex flex-wrap self-center mt-4  rounded-md w-full  ">
            ...
            <div className="flex flex-col p-2  my-3 pb-3 border-b border-gray-300">
                <div className="px-2 w-full flex pb-3 border-b border-b-gray-300">
                    <div className="flex max-sm:flex-col items-center">
                       ...
                       <button v-if="deleteSelectValues.length>0" className="py-1 hidden ml-2 px-2 bg-rose-700 text-white rounded-md text-sm capitalize" 
            ...
            <div className="relative overflow-x-auto  scr-container overflow-y-autحo">
                <div className="w-full text-sm text-left  rtl:text-right text-gray-500 ">
                     {showGrid && products && products?.length > 0 && 
                      ...
                      {showTable && products && products?.length > 0 && 
                    <TableView products={products} unSelected={unSelected} getSelected={getSelected}  />
                    } 
                    
                    {showGridCols && products && products?.length > 0 && 
                    <GridColsView products={products} unSelected={unSelected} getSelected={getSelected}  />
                    }  
                </div>
            </div>
