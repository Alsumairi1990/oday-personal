...
import { editTag, getTagByName} from '../_actions/Actions';
...
const FormEdit = ({ name, closeModel }: FormEditProps) => {
  ...
  type inputType = z.infer<typeof addSchema>;
  ...
  const getCategory = async () => {
    const tagName = await getTagByName(name);
    setTag(tagName);
  };
  ...
  useEffect(() => {
    ...
    if(tag?.image){setImageSrc(tag?.image);}
    if(tag?.icon){setIconSrc(tag?.icon);}
  ...
  return (
   <div className="fixed bg-[#0000003f] h-full flex overflow-hidden items-center justify-center w-full left-0 top-0 m-auto z-50 p-6 max-h[100vh]">
           {tag && <form onSubmit={handleSubmit(saveUser)} className='w-6/12 bg-white rounded-md shadow-2xl scr-container overflow-y-auto h-[95vh]' >
