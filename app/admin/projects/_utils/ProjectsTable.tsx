...
const ProjectsTable = ({categories,searchParam,getSelected,unSelected}:Props) => {
...
const addSelected= (selected:string)=>{
  ...
  return (
    ...
    <td className="flex-10 text-center flex justify-center py-4">
        {option?.image &&
        <img className='w-9 h-9 rounded-full p-0.5 border border-gray-200 ' src={`${baseUrl}/${option?.image}`} alt="" />
        
}

    </td>
    <td className="flex-20  flex justify-center  text-gray-600 font-medium py-4">
        {option.name}
    </td>
    ...
    <td className="flex-30 flex justify-center line-clamp-2 py-4">
           <p className="line-clamp-2 m-0 font-normal ">{option.description}</p> 
    </td>
