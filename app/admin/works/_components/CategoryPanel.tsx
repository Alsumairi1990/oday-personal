...
import { addServiceCategories, removeServiceCategory } from "../../service/_serviceActions/ServiceActions";
import { addWorkCategories, removeWorkCategory } from "../_actions/Actions";

interface FormEditProps {
  work: WorkWithModels;
  colseModel: (value: boolean) => void;
}
const CategoryPanel = ({ work, colseModel }: FormEditProps) => {
  ...
  const addWork = async () => {
    try {
        ...
        const result = await addWorkCategories(Number(work.id), numberArray);
  ...
  const removeElem = async ()=> {
    try {
        setLoading(true);
        const numberArray = selectedMenuElementsId.map(value => Number(value));
        const result = await removeWorkCategory(work.id,removedTool);
        setWorkData(result);
        setLoading(false);
       setSelectedMenuElements([]);
              ...
              <div>
                ...
                <span className="text-gray-600 text-md">List of added categories : </span>
                  <div className="flex items-center rounded-md w-full gap-2 max-h-16 flex-wrap overflow-y-auto">
                   ...
                   {workData.categories && workData.categories.length>0 ?  (
                          <div className="py-2 flex gap-2 items-center ">
                          {workData.categories.map(element => (
                            <div className="flex items-center  rounded-md pl-2 pr-1 bg-gray-50 border border-gray-200">
                                {element.category.name && <span className='text-sm text-orange-600 py-1 mr-2 font-medium'>{element.category.name}</span>}
                                <button
                                  onClick={() => {
                                    if(workData.categories.length>0) setRemovedTool(element.category.name);
