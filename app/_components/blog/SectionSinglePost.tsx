...
interface Props {
  category :  PostCategoryFront,
  locale : string,
  messages : AbstractIntlMessages,
}
const SectionSinglePost =  ({category,locale,messages}:Props) => {
    ...
    <section className="w-full">
            <div className="rtl:font-arabic">
                <div className="p-1">
                  ...
                  {category && category.pots.length > 0 && category.pots.slice(0,1).map((post) => (
                      ...
                      <div className="flex bg-white max-sm:flex-col px-2 pb-2 sm:pt-2 sm:rounded-md max-sm:rounded-b-md max-sm:pt-[1.4rem] ">
                        <div className="flex-50 max-sm:flex-100  max-sm:order-2">
                           <div className="mb-2">
                            ...
                            <h3 className="text-base font-semibold sm:leading-8"><Link href={`/blog/${post.title}`}>{post.titleAr}</Link> </h3>   
                        </div>
