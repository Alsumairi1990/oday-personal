...
import { ServiceClientInt } from '@/app/models/ServiceClientInt';


interface ServiceProps {
    serviceClient: ServiceClientInt;
  }
const ServiceClient = ({ serviceClient } : ServiceProps) => {
  return (
    <div className="flex-100 sm:flex-15 rounded-md p-2 bg-white dark:bg-black-150">
    <div className=" py-2 text-center rounded-md  border h-full border-gray-200">
     <div className="flex px-3  justify-center">
        <img src={serviceClient.image} alt="" />
     </div>
        <div className="">
            {/* <p className="text-gray-800 dark:text-orange-500 text-xl mb-1 font-semibold ">{serviceClient.name}</p> */}
            {/* <p className="text-gray-500 text-sm dark:text-gray-200 tracking-8  leading-6 px-1.5 font-medium mb-4">{serviceClient.desc}</p> */}
