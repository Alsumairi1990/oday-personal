...
const ServicesFeature = ({ servicefeature,locale, messages} : ServiceProps) => {
  ...
  return (
    <div className="flex-100 sm:flex-20 rounded-md sm:p-2 bg-white dark:bg-[#151515]">
      <div className=" py-3 max-sm:flex text-center rounded-md shadow-xl border h-full border-gray-200">
        <div className="flex max-sm:flex-25 px-2 sm:mb-4  justify-center">
            {servicefeature.image && <img src={servicefeature.image} alt={servicefeature.name!} /> }
        </div>
        {locale == 'en' ?  <div className="mt-1.5 max-sm:flex-75 ">
              ...
              <p className="text-gray-500 text-sm dark:text-gray-200 tracking-8  leading-6 px-1.5 font-medium line-clamp-5 mb-4">{servicefeature.desc}</p>
                  ...
                  <p className="text-gray-800 dark:text-orange-500 text-md sm:text-xl mb-2 font-semibold ">{servicefeature.titleAr}</p>
                  <p className="text-gray-500 text-sm dark:text-gray-200 tracking-8 leading-6 line-clamp-2 sm:leading-6 px-1.5 font-medium sm:line-clamp-5 mb-1 sm:mb-4">{servicefeature.descAr}</p>
              </div>
            }
      </div>
    </div>
