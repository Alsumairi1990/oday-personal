...
const ServiceDisplayPanel = ({services,locale,messages}:Props) => {
                ...
                <div className="grid grid-cols-4 gap-6">
                     {showGrid && services && services?.length > 0 && 
                         services.map((service) => (
                            <ServiceCol service={service} locale={locale} messages={messages} />
                         ))
                        }
                        </div>
                    ...
                    <div className="space-y-5">
                     {services.map((service) => (
                        <ServiceGrid service={service} locale={locale} messages={messages}  />
                     ))}
                     </div>
                    }
...
export default ServiceDisplayPanel;
