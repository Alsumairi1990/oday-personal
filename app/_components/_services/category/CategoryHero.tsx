...
interface ServiceProps {
    category: ServiceCategoryInt;
  }
const CategoryHero = ({ category }: ServiceProps) => {
  return (
        <div className="w-full " >  
        <div className=" max-sm:min-h-lvh pt-[100px] sm:pb-8 flex px-4 relative w-full bg-no-repeat bg-center bg-cover -z-0" style={{backgroundImage: `url(${category.image})`}}>
        <div className="absolute top-0 h-full w-full left-0" style={{backgroundImage: 'linear-gradient(to top, rgba(0, 0, 0, 0.3), rgb(8 8 8))'}}></div>
          <div className="flex w-11/12 mx-auto relative">
             <div className="flex-60">
                ...
                <p className="text-sm text-gray-100 font-medium leading-8">{category.desc}</p>
