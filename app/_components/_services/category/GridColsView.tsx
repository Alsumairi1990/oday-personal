...
import { ServiceOfferInt } from '@/app/models/ServiceOfferInt';
import { Service } from '@prisma/client';
interface ServiceProps {
    serviceOffer: Service;
  }
const ServiceOffer = ({ serviceOffer } : ServiceProps) => {
  return (
    <div className="flex-100 sm:flex-20 p-2">
    <div className=" py-3 text-center rounded-md shadow-xl border border-gray-200">
     <div className="flex px-2  justify-center">
        {serviceOffer.image && <img src={serviceOffer.image} alt={serviceOffer.name} />}
     </div>
        <div className="mt-1.5 ">
            <p className="text-gray-800 text-xl mb-1 font-semibold ">{serviceOffer.name}</p>
            <p className="text-gray-500 text-sm tracking-8  leading-6 px-1.5 font-medium mb-4">{serviceOffer.description}</p>
