...
const ServicesFull = async ({services,meta}:Props) => {
  ...
  return (
      ...
      <div className="flex flex-col sm:py-8   dark:bg-[#080808]">
        ...
        : <h2 className="sm:text-3xl text-lg font-arabic rtl:font-bold text-center px-8 mb-2 text-blue-700 dark:text-white font-normal">{meta.titleAr}</h2>
        }
        ...
        <div className="grid grid-cols-2 sm:grid-cols-6 gap-x-5 gap-y-8 mt-8">
          {services && services.map((service) => (
              <Link  href={`/services/${service.name_slug}`} className="p-2 flex flex-col justify-center py-4 border-dashed border-gray-400 bg-white sm:pt-2 dark:bg-[#111] hover:bg-orange-500 hover:text-white text-orange-600 dark:text-[#ca82ef] border shadow-lg dark:shadow-0 dark:max-sm:bg-[#171717] rounded-xl dark:border-gray-600 dark:sm:border-[#8849a9]">
              <div className="dark:pb-2 pb-3 pt-2 dark:pt-0 dark:sm:pb-2 w-24    sm:w-[7rem] dark:sm:w-[7rem] rounded-xl  px-4 dark:px-0 flex items-center mx-auto ">
