...
const ContactForm = ({locale,messages}:Props) => {
    ...
    const [projectType, setPorjectType] = useState<boolean>(false);
    const email = (messages as any).Common.email;
    const userName = (messages as any).Common.userName;
    const phoneCall = (messages as any).Common.phoneCall;
    const submit = (messages as any).HomePage.ContactUs.submit;
    const contactPart1 = (messages as any).Common.contactPart1;  
    const contactPart2 = (messages as any).Common.contactPart2;  
    const communicationMethod = (messages as any).Common.communicationMethod;  
    const videoCall = (messages as any).Common.videoCall; 
    const yourMessage = (messages as any).Common.yourMessage;  
    const messagePlacholder = (messages as any).Common.messagePlacholder;  
    const contactUsthrough = (messages as any).Common.contactUsthrough;    
    const sam = (messages as any).Common.sam;  
    const chooseService = (messages as any).Common.chooseService;  
  ...
  return (
    <div className="shadow-md bg-white dark:bg-card border dark:border-gray-600 rounded-xl w-11.3/12 sm:w-10/12 mx-auto  " >
     <div className="grid grid-cols-1 sm:grid-cols-3 ">
        <div className="ltr:pr-8 rtl:pl-8 max-sm:hidden">
            <div className="bg-[#162d42] dark:bg-black-100 dark:border-r bg-no-repeat bg-center bg-cover ltr:rounded-l-xl rtl:rounded-r-xl dark:border-r-gray-800 h-full" style={{backgroundImage:`url(${imagePath1})`}}>
                ...
                <div className="flex  justify-between pb-1 px-4">
                    <div className=" flex flex-col flex-100 border rounded border-[rgba(0,0,0,0.125)] p-2 text-center text-sm font-semibold col-6 flex-fill">
                        ...
                        <p className="text-lead-feature flex-50 text-md text-white dark:text-gray-900 font-weight-semibold mb-0">{contactUsthrough}</p>
        ...
        <div className="col-span-2 max-sm:p-2 sm:ltr:pr-4 sm:rtl:pl-8 ">
            <div className="flex flex-col items-center mb-2 justify-center">
                ...
                <h1 className="text-black-150 dark:text-white text-center text-2xl max-sm:text-lg font-semibold">{contactPart1} <span className="text-orange-400">{sam}</span> {contactPart2}</h1>
            </div>
            <div className=" border-b border-b-gray-200 dark:border-gray-800 pb-5">
                ...
                <div className=" flex items-center z-0 w-full mb-5 group">
                    <span className=" border bg-gray-100 dark:bg-card ltr:border-r-0 ltr:rounded-l-md rtl:border-l-0 rtl:rounded-r-md border-gray-300 dark:border-gray-600 h-10 flex items-center px-2">
                    ...
                    <input type="text" name="user_name" id="user_name" className="block ltr:pl-2 rtl:pr-2 h-10 px-0 z-0 w-full text-sm text-gray-900 bg-transparent border rounded-md ltr:rounded-l-none rtl:rounded-r-none border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-orange-500 peer" placeholder="" required />
                    <label htmlFor="user_name" className="ltr:ml-2 rtl:mr-2 peer-focus:font-medium absolute text-sm  text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-6 scale-75 top-3 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-orange-500 peer-focus:py-0.5 bg-white dark:bg-card  peer-focus:dark:text-orange-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 ">{userName}</label>
                    </div>
                </div>
                <div className=" flex items-center z-0 w-full mb-5 group">
                    <span className=" border bg-gray-100 dark:bg-card ltr:border-r-0 ltr:rounded-l-md rtl:border-l-0 rtl:rounded-r-md border-gray-300 dark:border-gray-600 h-10 flex items-center px-2">
                    ...
                    <div className="relative flex w-full">
                    <input type="email" name="floating_email" id="floating_email" className="block ltr:pl-2  rtl:pr-2 h-10 px-0 z-0 w-full text-sm text-gray-900 bg-transparent border ltr:rounded-md rtl:rounded-r-none ltr:rounded-l-none border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-orange-500 peer" placeholder="" required />
                    <label htmlFor="floating_email" className="ltr:ml-2 rtl:mr-2 peer-focus:font-medium absolute text-sm  text-gray-500 dark:text-gray-400 duration-300 transform -translate-y-9 scale-75 top-3 origin-[0] peer-focus:start-0 rtl:peer-focus:translate-x-1/4 rtl:peer-focus:left-auto peer-focus:text-orange-500 peer-focus:py-0.5 bg-white dark:bg-card  peer-focus:dark:text-orange-500 peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-6 ">{email}</label>
                    </div>
                </div>
                <div className="flex z-10 h-10 w-full dark:!bg-card " dir='ltr'>
                <PhoneInput 
                inputClass='!w-full dark:!bg-card '
                country={'us'}
                value={phoneNumber}
                ...
                <div className=" flex relative flex-col h-10 text-gray-600 l border border-gray-300 dark:border-gray-600 rounded-md w-full group">
                    <div className="flex items-center overflow-hidden">
                      <span className="w-12 ltr:border-r rtl:border-l border-gray-300 dark:border-gray-600  ltr:rounded-l-lg rtl:rounded-r-lg justify-center h-10 flex items-center bg-gray-100 dark:bg-card">
                        <svg className="w-6 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M4 6H20M7 12H17M9 18H15" stroke="#000000" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                       </span>
                      <button
                       onClick={() => { setPorjectType((prev) => !prev)}}
                       id="dropdownRadioHelperButton" data-dropdown-toggle="dropdownRadioHelper" className="text-gray-500 relative w-full  hover:bg-gray-100 focus:outline-none font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-center dark:bg-card dark:hover:bg-blue-700 dark:focus:ring-blue-800" type="button"> {chooseService} 
                       <svg className="w-2.5 h-2.5 ms-3 absolute ltr:right-2 rtl:left-2" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 10 6">
                        <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 4 4 4-4"/>
                         </svg>
                       </button>
                    </div>
                    {projectType && <div id="dropdownRadioHelper" className="!z-50 max-h-56  overflow-y-auto absolute top-11 sm:left-10 bg-white divide-y border border-gray-200 w-full divide-gray-100 rounded-lg shadow sm:w-72 dark:bg-gray-700 dark:divide-gray-600">
            ...
            <div className="sm:p-2 pt-4 ">
                <div className="">
                    <h2 className="text-sm font-semibold text-gray-700">{communicationMethod}</h2>
                    <div className="py-2 grid grid-cols-2 max-sm:gap-y-1 sm:grid-cols-3 items-center">
                        <div className="">
        ...
        <div className="">
            ...
            <label htmlFor="message" className="block mb-2 text-sm font-medium text-gray-600 dark:text-white">{yourMessage}</label>
            <textarea id="message" rows={2} className="block p-2.5 w-full text-sm text-gray-100 bg-gray-50 dark:bg-card rounded-lg border border-gray-300 focus:ring-violet-600 focus:border-violet-700 dark:bg-black-150 dark:border-gray-800 dark:placeholder-gray-400 dark:text-white dark:focus:ring-violet-600 dark:focus:border-violet-700" placeholder={messagePlacholder}></textarea>

        </div>
     </div>
     <div className="my-2 p-2 text-center">
     <a href="" className="bg-orange-500 dark:bg-violet-600 rounded-md px-6 py-1.5 text-white inline-block">{submit}</a>
     </div>
