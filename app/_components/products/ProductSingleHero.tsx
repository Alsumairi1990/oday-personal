...
import { HeroSection, Market } from '@prisma/client';
import { ProductForFront } from '@/app/[locale]/admin/products/_utils/ProductForFront';
interface ServiceProps {
    product: ProductForFront;
    locale : string,
    messages : AbstractIntlMessages,
    page : string,
  }
const ProductSingleHero = ({ product, locale,messages,page }: ServiceProps) => {
    const talkToConsultant = (messages as any).CategoryPage.talkToConsultant;
    const home = (messages as any).Common.home;
  return (
        <div className="w-full " >  
        <div className=" sm:h-[calc(100%-350px)] hero-section pt-[100px] sm:pb-8 flex px-4 relative w-full bg-no-repeat bg-center bg-cover -z-0" style={{backgroundImage: `url(${product.image})`}}>
          ...
          <div className="flex w-11/12 mx-auto relative">
             <div className="flex-100 sm:flex-60">
               ...
               <>
                ...
                <h2 className="sm:text-3xl text-xl text-white font-extrabold mb-4">{product.name}</h2>
                <h2 className="text-md sm:text-md leading-8 sm:leading-8 text-white font-bold line-clamp-2 sm:line-clamp-2  mb-4">{product.description}</h2>
                </>
                :
                <>
                <span className="text-[13px] font-arabic flex  items-center text-white mb-6">{home} <span className='text-[12px] px-2'>|</span> {page} </span>
                <h2 className="sm:text-3xl text-xl text-white font-extrabold font-arabic mb-4">{product.nameAr}</h2>
                <h2 className="text-md sm:text-md leading-8 sm:leading-8 text-white font-bold line-clamp-2 sm:line-clamp-2 font-arabic mb-4">{product.descriptionAr}</h2>
                </>
                }
                <div className="flex mt-2 sm:mt-6 items-center  ">
