...
const HeroSection = ({ market, locale,messages }: ServiceProps) => {
  ...
  return (
        ...
        <div className=" sm:h-[calc(100%-350px)] hero-section pt-[100px] sm:pb-8 flex px-4 relative w-full bg-no-repeat bg-center bg-cover -z-0" style={{backgroundImage: `url(${market.image})`}}>
           <div className="absolute sm:top-[75px] h-[calc(100%-75px)] sm:h-[calc(100%-75px)] top-16 bottom-0 border-t border-t-gray-800  w-full left-0" style={{backgroundImage: 'linear-gradient(to top, rgb(8 8 8),rgb(0 0 0 / 55%), rgb(8 8 8))'}}></div>
          ...
          <div className="flex w-11/12 mx-auto relative">
             <div className="flex-60">
               ...
               <>
                ...
                <h2 className="sm:text-4xl text-xl text-white font-extrabold mb-4">{market.title}</h2>
                </>
                :
                <>
                <span className="text-sm text-white mb-6 inline-block">{home} / {market.titleAr} </span>
                <h2 className="sm:text-4xl text-xl text-white font-extrabold font-arabic mb-4">{market.titleAr}</h2>
