...
const Blogs = async ({meta,posts}:Props) => {
  ...
  return (
   <div className="flex w-[90%] mx-auto  items-center flex-col my-8">
      +
      <div className="p-2">
      {locale == 'en' ? <h2 className="text-base sm:text-3xl font-bold text-blue-600 dark:text-white mt-3 text-center">{meta.title}</h2>
       : <h2 className="text-base sm:text-3xl font-bold font-arabic text-blue-600 dark:text-white mt-3 text-center">{meta.titleAr}</h2>
       }
      <div className="grid sm:grid-cols-2 gap-3 mt-8">
        ...
        {posts && 
            ...
            posts.map((post) => (
                ...
                {post.image && <img className='w-full h-full mx-auto' src={post.image} alt="" />}
                ...
                <div className="p-1 bg-[#1c1c1c] flex flex-col">
                    ...
                    <div className="p-4">
                        {locale == 'en' ? (
                            <>
                            <h2 className="text-gray-100 mb-2 text-base font-semibold">{post.title}</h2>
                                 <p className="text-[13px]  font-normal text-gray-200 line-clamp-4">{post.content}
                            </p>
                           </>
                        )
                        : ( 
                            <>
                        <h2 className="text-gray-100 font-arabic mb-2 text-base font-semibold">{post.titleAr}</h2>
                    ...
                    {locale == 'en' ? <Link href={'/services'} className="text-gray-200 capitalize text-[13px] px-2 py-1.5 border  border-gray-400 ">{meta.more} </Link>
                  : <Link href={'/services'} className="text-gray-200 font-arabic font-semibold capitalize text-[12px] px-2 py-0.5 border  border-gray-400 "> {meta.moreAr.split(' ')[0]}  ... </Link> 
      ...
      <div className="mt-8 flex justify-center">
        ...
        {locale == 'en' ? <Link href={'/blog'} className="capitalize text-gray-700  px-2.5 rounded py-1.5 font-semibold bg-gray-100 ">{meta.more} </Link>
                  : <Link href={'/blog'} className="capitalize font-arabic border border-gray-200 text-gray-700  px-2.5 rounded py-1.5 font-semibold bg-gray-100 "> {meta.moreAr} </Link> 
                  // <h2 className="text-xl  rtl:font-bold rtl:mb-3 text-center px-3 mb-6 text-blue-700 dark:text-white font-normal">{service.nameAr}</h2>
                  }
        </div>
