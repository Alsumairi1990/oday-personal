...
const BlogsTable = ({posts,searchParam,getSelected,unSelected}:Props) => {
...
const addSelected= (selected:string)=>{
  ...
  return (
    ...
    <td className="flex-5 flex items-center justify-center ">
       <button 
            onClick={()=> {setPost(post),setShowDisplayModel(true)}}
    ...
    <td className="text-center flex-5  font-semibold py-3">
            ...
            <label className="relative flex items-center pt-1 rounded-full cursor-pointer" htmlFor="checkbox">
                <input type="checkbox"
                    ...
                    onChange={(e) => {
                        ...
                        if (isChecked) {
                        getSelected(String(post.id));
                        } else {
                            setSelectedValues(selectedValues.filter(rowId => rowId !== String(post.id)));
                        unSelected(String(post?.id));
                        }
                    }}
                    />
                <span
    ...
    <td className="flex-15 text-center flex justify-center py-3">
        {post?.image &&
        <div className="p-1 rounded-md">
          <img className='w-20 h-14 rounded-md p-0.5 border border-gray-200 ' src={`${baseUrl}/${post?.image}`} alt="" />
        </div>
        
}

    </td>
    <td className="flex-20  flex justify-center  text-gray-600 font-medium py-3">
        {post.titleAr}
    </td>
    <td className="flex-15 flex justify-center line-clamp-2 py-3">
           <span className=" font-normal ">
            {post.slug}
        </span> 
    </td>
    <td className="flex-15  flex justify-center py-4">
        {post && post.categories.length > 0 && post.categories.map((category)=> (<span className='mx-1'>{category.nameAr}</span>))}
    </td>
    ...
    <td className="flex-10 flex  text-center py-3 justify-around">
    <Link href={`/blog/edit/${post.slug}`} className="inline-flex items-center justify-center bg-sky-100 border !border-sky-200 hover:!bg-sky-200 rounded-md py-1.5 flex-23">
    ...
    <Link href={`/blog/show/${post.slug}`} className="inline-flex items-center justify-center bg-blue-100 border !border-blue-200 hover:!bg-blue-200 rounded-md flex-23">
