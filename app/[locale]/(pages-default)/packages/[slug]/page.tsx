...
const PackageCategorySinglePage = async ({params}:Props) => {
  ...
  const categoryData = await fetch(`${process.env.NEXTAUTH_URL}/api/front/packages/categories/${params.slug}`, {
   method: 'GET',
   next: { revalidate: 3600 }, 
 });
  const category:PackageCatForFront = await categoryData.json();
           ...
           <div className="w-11/12 my-10 mx-auto">
              <div className="grid sm:grid-cols-3 sm:gap-x-12 max-sm:gap-y-6  w-full">
                {category && category.packages && 
                   category.packages.map((elem) => (
                    <PackageCard  packageData = {elem} locale = {locale} messages = {messages} categorySlug={params.slug} />
                   ))
                }
              </div>
...
export default PackageCategorySinglePage;