...
const planSinglePage = async ({params}:Props) => {
  ...
  const page = `${plans}`;
  const planData = await fetch(`${process.env.NEXTAUTH_URL}/api/front/plans/${params.plan}`, {
   method: 'GET',
   next: { revalidate: 3600 }, 
   });
   const no:number = 3;
                                                                 // api/front/plans/categories/web-development-plans/plans/1
   const similarPlanData = await fetch(`${process.env.NEXTAUTH_URL}/api/front/plans/categories/${params.slug}/plans/similar/3`, {
    method: 'GET',
    next: { revalidate: 3600 }, 
  });
  const plan:PlanForFront = await planData.json();
  const similarPlan:Plan[] = await similarPlanData.json();
   ...
   <>
           ...
           <div className="my-14 rtl:font-arabic w-11/12 mx-auto">
            ...
            <div className="flex mt-3 border  border-gray-300 flex-wrap rounded-md">
              <div className="flex-100 sm:h-16 rtl:pr-1  flex border-b border-b-gray-300 items-center rounded-md ">
                <span className="text-base h-[90%] flex justify-center items-center flex-35 sm:flex-15  px-2 font-semibold w-full text-center py-1.5 rounded-md text-gray-800 dark:text-gray-50 bg-gray-200 dark:bg-gray-800">{features}</span>
                <span className="text-md flex-65 rtl:pr-7 font-semibold mt-1 text-gray-700 max-sm:leading-6 py-2">{plan.featuresAr}</span>
              </div>
              <div className="flex-100 sm:h-16 rtl:pr-1  border-b border-b-gray-300 flex items-center rounded-md ">
                <span className="text-base h-[90%] flex justify-center items-center flex-35 sm:flex-15  px-2 font-semibold w-full text-center py-1.5 rounded-md text-gray-800 dark:text-gray-50 bg-gray-200 dark:bg-gray-800">{limits}</span>
                <span className="text-md flex-65 rtl:pr-7 font-semibold mt-1 text-gray-700 max-sm:leading-6 py-2">{plan.limitsAr}</span>
              </div>
              <div className="flex-100 sm:h-16 rtl:pr-1  border-b border-b-gray-300 flex items-center rounded-md ">
                <span className="text-base h-[90%] flex justify-center items-center flex-35 sm:flex-15  px-2 font-semibold w-full text-center py-1.5 rounded-md text-gray-800 dark:text-gray-50 bg-gray-200 dark:bg-gray-800">{support}</span>
                <span className="text-md flex-65 rtl:pr-7 font-semibold mt-1 text-gray-700 max-sm:leading-6 py-2">{plan.supportAr}</span>
              </div>
              <div className="flex-100 sm:h-16 rtl:pr-1  border-b border-b-gray-300 flex items-center rounded-md ">
                <span className="text-base h-[90%] flex justify-center items-center flex-35 sm:flex-15  px-2 font-semibold w-full text-center py-1.5 rounded-md text-gray-800 dark:text-gray-50 bg-gray-200 dark:bg-gray-800">{purpose}</span>
                <span className="text-md flex-65 rtl:pr-7 font-semibold mt-1 text-gray-700 max-sm:leading-6 py-2">{plan.purposeAr}</span>
              </div>
              <div className="flex-100 sm:h-16 rtl:pr-1  border-b border-b-gray-300 flex items-center rounded-md ">
           ...
           <div className="my-14 w-11/12 mx-auto ">
            <h2 className="text-gray-800 text-center text-xl dark:text-gray-50 max-sm:text-base  font-semibold mb-3 px-2">Similar Plans</h2>
            <div className="grid sm:grid-cols-4 justify-center gap-x-8 w-full">
              {similarPlan.length > 0 && similarPlan.map((plan) => (
                            <PlanCardSimilar  plan={plan} locale={locale}  messages={messages}  />
              ))}
            </div>
