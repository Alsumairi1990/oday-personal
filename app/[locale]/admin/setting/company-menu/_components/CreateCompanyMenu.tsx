...
import { createCompanyMenu } from '../_actions/Actions';

type inputType = z.infer<typeof CompanyMenuSchema>;
...
const CreateCompanyMenu = () => {
    ...
    resolver: zodResolver(CompanyMenuSchema),
  });
  const rows = 4;
  const cols = 4;
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const saveUser: SubmitHandler<inputType> = async (data)=>{
      ...
      try {
        setLoading(true);
        const codeData =  await createCompanyMenu(formData);
                ...
                <div className=" flex flex-wrap justify-between px-5 relative  pt-6 ">
                  ...
                  <div className="p-4 w-full flex flex-wrap justify-between">
                    ...
                    <div className=" flex sm:flex-48 flex-col z-0 w-full mb-5 group">
                            ...
                            <div className="flex items-center w-full">
                                <div className="relative flex w-full">
                                <input {...register('nameAr')}  type="text" name="nameAr" id="nameAr" className="block pl-2 h-10 px-0 z-0 w-full text-sm text-gray-900 border rounded-xl border-gray-300 appearance-none  bg-gray-50 focus:outline-none focus:ring-0 focus:border-orange-500 peer" placeholder="Section name ...." required />
                                </div>
                            </div> 
                    ...
                    <div className=" flex sm:flex-100 flex-col z-0 w-full mb-5 group">
                            <label htmlFor="url" className="font-medium mb-1.5 text-sm  text-gray-700 duration-300 capitalize">   Page URL</label>
                            <div className="flex items-center w-full">
                                <div className="relative flex w-full">
                                <input {...register('url')}  type="text" name="url" id="url" className="block pl-2 h-10 px-0 z-0 w-full text-sm text-gray-900 border rounded-xl border-gray-300 appearance-none  bg-gray-50 focus:outline-none focus:ring-0 focus:border-orange-500 peer" placeholder="Page URL ...." required />
                                </div>
                            </div> 
                            <span className="text-red-400 text-xs mt-2">{errors.nameAr?.message} </span>
                    </div>
                    ...
                    <div className=" flex sm:flex-48 flex-col z-0 w-full mb-5 group">
                            ...
                            <div className="flex items-center w-full">
                                <div className="relative flex w-full">
                                <textarea {...register('description')} cols={cols} rows={rows}   name="description" id="description" className="block bg-gray-50 pl-2 pt-2 px-0 z-0 w-full text-sm text-gray-900  border rounded-xl border-gray-300 appearance-none  focus:outline-none focus:ring-0 focus:border-orange-500 peer" placeholder="  Decription ...." required />
                                </div>
                            </div> 
