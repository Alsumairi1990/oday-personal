...
import DeleteService from '@/app/[locale]/admin/service/_components/DeleteService';
import { getMenuElements } from '../_actions/Action';
import { MenuWithModels } from '../_utils/MenuWithModels';
import DeleteMenuElement from './DeleteMenuElement';


const MenuDelete = () => {
    
    
    const [menuElements, setMenuElements] = useState<MenuWithModels[]>([]);
  ...
  const loadMenuElements = async () => {
    try {
      setLoading(true);
      const catgs = await getMenuElements();
   ...
   <div className="w-full sm:w-11.8/12 max-sm:border relative max-sm:border-gray-300  m-auto p-6 bg-white border border-gray-300 rounded-md">
    {loading && <div className=' w-full h-full z-40 bg-[#00000012] absolute top-0 left-0  flex items-center justify-center' style={{backdropFilter: 'blur(2px)'}}><div className='loader-2 w-4'></div></div>}
     <div className="text-start z-40  ">
      ...
      {showDelete && <DeleteMenuElement closeModel={closeConfirm} ids={svalues}  />}  
                    ...
                    <div className=" w-full h-16  sm:flex sm:mx-auto items-center bg-white border-r border-r-gray-300 rounded-l-md">
                            ...
                            <div className="pl-4  w-full">
                                <div className="w-full mb-2 flex items-center">
                                    <span className="text-base  text-black  font-medium">{element?.title}</span>                           
                                </div>
                            </div>
                        </div>
                    </div>
