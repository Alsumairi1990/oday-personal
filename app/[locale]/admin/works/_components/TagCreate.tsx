...
import { createServiceCategory, createServiceTool, createWorkCategory, createWorkTag, getTags, removeServiceCategory, removeServiceTool, removeWorkCategory, removeWorkTag } from '../../common/_actions/Actions';
import { IoMdCloseCircle } from "react-icons/io";
import { addWorkCategory, addWorkTag, getWorkWCategorylById, getWorkWTagsById } from '../_actions/Actions';
...
const getSelected= (selected:string)=>{
   ...
   const addTag = async () => {
    try {
      ...
      setLoading(true);
      const names = await addWorkTag(workId, numberArray);
      setLoading(false);
      setTagNames(names);
  ...
  const getTagNames = async ()=> {
    try {
        setLoading(true);
        const names = await getWorkWTagsById(workId);
        setLoading(false);
        setTagNames(names);
        setError(null); 
  ...
  const removeTag  = async () => {
    try {
      setLoading(true);
      const tools = await removeWorkTag(workId,removedTool);
      setLoading(false);
     setTagNames(tools);
     setError(null);
    ...
    const createTag = async () => {
      try {
        if(workId != 0 && tagName != ''){
          setLoading(true);
          const catname = await createWorkTag(workId , tagName);
          setLoading(false);
          setError(null)
