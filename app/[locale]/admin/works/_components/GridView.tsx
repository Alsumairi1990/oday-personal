import { Location, Service, Work} from '@prisma/client';
...
const GridView = ({works,getSelected,unSelected}:Props) => {
     ...
     works.map((element:any) => (
        <div className=" relative border flex flex-wrap my-4 w-11.8/12 mx-auto items-center  border-gray-200 rounded-md " >
           ...
           <WorkCard   element={element} />
            ...
            <Link href={`/admin/works/edit/${element.id}`} className="inline-flex items-center justify-center bg-blue-100 border !border-blue-200 hover:!bg-blue-200 rounded-md py-1 flex-23">
            ...
            {/* {editShow && <FormEdit name={category.name} closeModel={closeModel}  /> } */}
                <Link href={`/admin/service/show/${element.id}`} className="inline-flex items-center justify-center bg-blue-100 border !border-blue-200 hover:!bg-blue-200 rounded-md py-1 flex-23">
