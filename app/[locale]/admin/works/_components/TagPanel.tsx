...
import { addWorkCategories, addWorkTags, removeWorkCategory, removeWorkTag } from "../_actions/Actions";
...
const TagPanel = ({ work, colseModel }: FormEditProps) => {
  ...
  const addWork = async () => {
    try {
        ...
        const result = await addWorkTags(Number(work.id), numberArray);
  ...
  const removeElem = async ()=> {
    try {
        setLoading(true);
        const numberArray = selectedMenuElementsId.map(value => Number(value));
        const result = await removeWorkTag(work.id,removedTool);
        setWorkData(result);
        setLoading(false);
       setSelectedMenuElements([]);
                   ...
                   {workData.tags && workData.tags.length>0 ?  (
                          ...
                          {workData.tags.map(element => (
                            <div className="flex items-center  rounded-md pl-2 pr-1 bg-gray-50 border border-gray-200">
                                ...
                                <button
                                  onClick={() => {
                                    if(workData.tags.length>0) setRemovedTool(element.tag.name);
