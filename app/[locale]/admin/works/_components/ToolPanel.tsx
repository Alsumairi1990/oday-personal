...
const ToolPanel = ({ work, colseModel }: FormEditProps) => {
  ...
  const addWork = async () => {
    try {
        ...
        const result = await addWorkTools(Number(work.id), numberArray);
  ...
  const removeElem = async ()=> {
    try {
        setLoading(true);
        const numberArray = selectedMenuElementsId.map(value => Number(value));
        const result = await removeWorkTool(work.id,removedTool);
        setWorkData(result);
        setLoading(false);
       setSelectedMenuElements([]);
              ...
              <div>
                ...
                <span className="text-gray-600 text-md">List of added Tools : </span>
                  <div className="flex items-center rounded-md w-full gap-2 max-h-16 flex-wrap overflow-y-auto">
                   ...
                   {workData.tools && workData.tools.length>0 ?  (
                          <div className="py-2 flex gap-2 items-center ">
                          {workData.tools.map(element => (
                            <div className="flex items-center  rounded-md pl-2 pr-1 bg-gray-50 border border-gray-200">
                                ...
                                <button
                                  onClick={() => {
                                    if(workData.tools.length>0) setRemovedTool(element.tool.name);
                        ...
                        <div className="pb-1">
                        <span className="text-sm text-orange-600 px-4 inline-flex capitalize">No Added Tools  </span>
                    </div>                                   
                      )
                      }
                  </div>
