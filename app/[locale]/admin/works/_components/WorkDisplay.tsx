...
import { getWorkWithModels } from '../_actions/Actions';
...
const WorkDisplay = () => {
   ...
   const [works, setWorks] = useState<WorkWithModels[] | null>(null);
   ...
   const getAllWorks = async () => {
    try {
        setLoading(true)
        const catgs = await getWorkWithModels();
        setWorks(catgs);
        setError('');
    } catch (error:any) {
        setLoading(false);
    ...
    {loading && <div className=' w-full h-full z-50 bg-[#00000012] absolute top-0 left-0  flex items-center justify-center' style={{backdropFilter: 'blur(2px)'}}><div className='loader-2 w-4'></div></div>}
            ...
            <div className="relative overflow-x-auto  scr-container overflow-y-autحo">
                <div className="w-full text-sm text-left  rtl:text-right text-gray-500 dark:text-gray-400">
                     {showGrid && works && works?.length > 0 && 
                        <GridView works={works} unSelected={unSelected} getSelected={getSelected}  />
                        }
