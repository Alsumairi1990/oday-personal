...
import { createServiceCategory, createServiceTool, createWorkCategory, removeServiceCategory, removeServiceTool, removeWorkCategory } from '../../common/_actions/Actions';
import { IoMdCloseCircle } from "react-icons/io";
import { addWorkCategory, getWorkWCategorylById } from '../_actions/Actions';
...
const CategoryCreate = ({workId}:Props) => {
...
const getSelected= (selected:string)=>{
   ...
   const addCategory = async () => {
    try {
      ...
      setLoading(true);
      const names = await addWorkCategory(workId, numberArray);
      setLoading(false);
  ...
  const getCategoryNames = async ()=> {
    try {
        setLoading(true);
        const names = await getWorkWCategorylById(workId);
        setLoading(false);
        setCategoryNames(names);
        setError(null); 
  ...
  const removeCategory  = async () => {
    ...
    try {
      setLoading(true);
      const tools = await removeWorkCategory(workId,removedTool);
      setLoading(false);
    ...
    const createCategory = async () => {
      try {
        if(workId != 0 && categoryName != ''){
          setLoading(true);
          const catname = await createWorkCategory(workId , categoryName);
          setLoading(false);
          setError(null)
