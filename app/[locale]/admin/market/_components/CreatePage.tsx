...
import { MenuParentSchema } from '../../setting/left-nav/_utils/MenuParentSchema';
import { addingMenuParent, getMenuParent } from '../../setting/left-nav/_actions/Action';

interface Props {
    id? : number
    closeModel? : (value:boolean) => void
}
type inputType = z.infer<typeof MenuParentSchema>;
const CreatePage = ({id,closeModel}:Props) => {
   ...
   const cols = 30;
    ...
    resolver: zodResolver(MenuParentSchema),
  });
  ...
  useEffect(() => {
    ...
    const saveMenu3: SubmitHandler<inputType> = async (data)=>{
      ...
      try {
        ...
        const result =  await addingMenuParent(formData);
        setElement(result)
     ...
     <form onSubmit={handleSubmit(saveMenu3)} className="text-start  z-40 rounded-md">
        <div className="flex flex-wrap justify-between ">
            ...
            <div className=" flex flex-100 flex-col z-0 w-full mb-5 group">
                    ...
                    <div className="flex items-center w-full">
                        <div className="relative flex w-full">
                        <input {...register('priority')}  type="number" name="priority" id="priority" className="block pl-2 h-10 px-0 z-0 w-full text-sm text-gray-900 border rounded-xl border-gray-300 appearance-none  bg-gray-100 focus:outline-none focus:ring-0 focus:border-orange-500 peer" placeholder="Phase title  ..." required />
                        </div>
                    </div> 
            ...
            <div className="flex flex-100 items-center  justify-center w-full">
                    <label htmlFor="iconIm" className="flex flex-col items-center justify-center w-full h-28 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-100  hover:bg-gray-100">
                        ...
                        <input id="iconIm" {...register('icon')}  type="file" name="icon" className="opacity-0 h-2 w-2" onChange={handleFileChange} />
                    </label>
