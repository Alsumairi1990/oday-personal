...
import { addBasic, addServiceCategory, addServiceTool, getAllServices, getCategories, getServiceWCategorylById, getServiceWToolById, getTools, updateServiceWithCategories } from '../_serviceActions/ServiceActions';
...
import { createServiceCategory, createServiceTool, removeServiceCategory, removeServiceTool } from '../../common/_actions/Actions';
...
const CategoryCreate = ({serviceId}:Props) => {
...
const getSelected= (selected:string)=>{
   ...
   const addCategory = async () => {
    try {
      ...
      setLoading(true);
      const names = await addServiceCategory(serviceId, numberArray);
      setLoading(false);
  ...
  const getCategoryNames = async ()=> {
    try {
        setLoading(true);
        const names = await getServiceWCategorylById(serviceId);
        setLoading(false);
        setCategoryNames(names);
        setError(null); 
  ...
  const removeCategory  = async () => {
    ...
    try {
      setLoading(true);
      const tools = await removeServiceCategory(serviceId,removedTool);
      setLoading(false);
    ...
    const createCategory = async () => {
      try {
        if(serviceId != 0 && categoryName != ''){
          setLoading(true);
          const catname = await createServiceCategory(serviceId , categoryName);
          setLoading(false);
          setError(null)
...
// import { useState } from 'react';
// import { toast } from 'react-toastify';
// import { addBasic, getAllServices, getCategories, updateServiceWithCategories } from '../_serviceActions/ServiceActions';
...
//     updateServiceWithCategories(serviceId,numberArray);
//    }
...
//     }, []);
//   return (
//     <div className="w-full sm:w-11.8/12 max-sm:border max-sm:border-gray-300 mx-auto p-6 bg-white  rounded-md">
//     <div className="text-start z-40  ">
...
//            <div className="flex flex-col max-h-72 mt-2 overflow-y-auto">
...
//                    <div className=" w-full h-12 sm:flex sm:mx-auto items-center bg-white border-r border-r-gray-300 rounded-l-md">
...
//                        </div>
//                    </div>
//                     <div className="flex gap-x-2 flex-20  justify-center items-center  ">
//                     <div className="inline-flex  z-20 bg-white  justify-center rounded-md">
//                         <label className="relative bg-whit justify-center flex items-center  rounded-full cursor-pointer" htmlFor="checkbox">
//                             <input type="checkbox"
//                                 className="before:content[''] peer relative h-[18px] w-[18px] cursor-pointer appearance-none rounded-md border !border-[#ccc] transition-all before:absolute before:top-2/4 before:left-2/4 before:block before:h-12 before:w-12 before:-translate-y-2/4 before:-translate-x-2/4 before:rounded-full before:bg-blue-gray-500 before:opacity-0 before:transition-opacity checked:border-indigo-600 checked:bg-indigo-600 checked:before:bg-indigo-600 hover:before:opacity-10"
...
//            </div>
//        </div>
//        {svalues.length>0 && <div className='sm:ml-4 my-3 border border-gray-200 py-2 items-center rounded-md flex justify-center'>
//             <div className="flex items-center">
//             <span className='text-violet-600 flex items-center font-medium border-b-2 text-md border-b-gray-400 mr-1 px-1'>{svalues.length}</span> 
//             <span className='text-md text-orange-500 font-medium'>
//                has been selected</span>
...
//             </div>
//              <button onClick = {addCategory}
//            type='button' className='inline-flex py-1  items-center px-2 rounded-md bg-violet-600 ml-4 text-white text-[14px] capitalize'>Add Category 
//              <span className="text-white ml-2 text-md"  >
//                <svg width="20px" height="20px" viewBox="0 0 15 15" fill="none" xmlns="http://www.w3.org/2000/svg">
//                   <path fill-rule="evenodd" clip-rule="evenodd" d="M1 14L1 1H2L2 14H1ZM9.85355 3.14645L14.2071 7.5L9.85355 11.8536L9.14645 11.1464L12.2929 8H3V7H12.2929L9.14645 3.85355L9.85355 3.14645Z" fill="#ffffff"/>
//                 </svg>
