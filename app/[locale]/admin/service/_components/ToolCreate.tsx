...
import { addBasic, addServiceTool, getAllServices, getCategories, getServiceWToolById, getTools, updateServiceWithCategories } from '../_serviceActions/ServiceActions';
...
import { createServiceTool, removeServiceTool } from '../../common/_actions/Actions';
...
const ToolCreate = ({serviceId}:Props) => {
...
const getSelected= (selected:string)=>{
   ...
   const addTool = async () => {
    try {
      ...
      setLoading(true);
      const names = await addServiceTool(serviceId, numberArray);
      setLoading(false);
      setToolNames(names);
  ...
  const getToolNames = async ()=> {
    try {
        setLoading(true);
        const names = await getServiceWToolById(serviceId);
        setLoading(false);
        setToolNames(names);
        setError(null); 
  ...
  const removeTool  = async () => {
    ...
    try {
      setLoading(true);
      const tools = await removeServiceTool(serviceId,removedTool);
      setLoading(false);
     setToolNames(tools);
     setError(null);
    ...
    const createTool = async () => {
      try {
        if(serviceId != 0 && toolName != ''){
          setLoading(true);
          const toolname = await createServiceTool(serviceId , toolName);
          setLoading(false);
          setError(null)
          setToolNames((prevToolNames) => [...prevToolNames, toolname]);
        }
    ...
    <div className="text-start z-40  ">
       <div className=" mb-3 border border-gray-200 rounded-md">
            <div className="px-2 border-b flex gap-2 items-center border-b-gray-200 py-1.5 ">
                ...
                <span className="text-gray-600 font-medium text-md">Tools added </span>
            </div>
          {toolNames.length>0 ?  (
      ...
      <div className="border-t my-1.5 flex items-center justify-center pt-3 border-t-gray-200 ">
        <div className="flex items-center border gap-x-3 py-1 border-gray-200  px-2 bg-gray-100 rounded-md">
          ...
          <span className="text-md inline-flex text-indigo-600">Selecet Tools</span>
        </div>
