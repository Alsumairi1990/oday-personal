...
import { deleteServices } from '../_serviceActions/ServiceActions';

interface Props {
    ids: string[];
    closeModel:(value : boolean) => void
  }
const DeleteService = ({ids,closeModel}:Props) => {
  ...
  const deleteRows = async () => {
    try {
        setLoading(true);
        const results = await deleteServices(ids);
         setResult(results);
         setLoading(false);
    } catch (error:any) {
        setLoading(false);
        ...
        {result != 0 && 
                ...
                <div className="flex items-center bg-green-500 py-2.5 px-2 rounded-t-md">
                 ...
                 <div className="py-3 border-t border-t-gray-200 flex justify-center">
                  <Link href="/admin/service/delete" className='text-sm bg-gray-300 px-1.5 py-.05 rounded-md'>
                    back 
                  </Link>
                 </div>
                 </div>     
