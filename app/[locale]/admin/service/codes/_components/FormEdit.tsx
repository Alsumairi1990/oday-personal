...
import { editCode, getCodeByName } from '../_actions/Actions';
...
const FormEdit = ({ name, closeModel }: FormEditProps) => {
  ...
  const saveUser: SubmitHandler<inputType> = async (data) => {
    ...
    console.log("name at form"+data.code);
    ...
    try {
      if (code?.id !== undefined) {
        await editCode(formData, code?.id);
      }              
    } catch (error) {
  ...
  const getCategory = async () => {
    const namedata = await getCodeByName(name);
    setCode(namedata);
  };
  ...
  useEffect(() => {
    ...
    if(code?.image){setImageSrc(code?.image);}
    if(code?.icon){setIconSrc(code?.icon);}
  ...
  return (
   <div className="fixed bg-[#0000003f] h-full flex overflow-hidden items-center justify-center w-full left-0 top-0 m-auto z-50 p-6 max-h[100vh]">
           {code && <form onSubmit={handleSubmit(saveUser)} className='w-6/12 bg-white rounded-md shadow-2xl scr-container overflow-y-auto h-[95vh]' >
