import { Category, ServiceCode, Tool } from '@prisma/client';
...
const GridView = ({codes,getSelected,unSelected}:Props) => {
    ...
    <div>
    {
     codes.map((element:any) => (
        <div className=" relative border flex flex-wrap my-4 w-11.8/12 mx-auto items-center  border-gray-200 rounded-md " >
           ...
           <TagCard   code={element} />
           </div>
            <div className="flex flex-100 sm:flex-20 gap-x-2  justify-center  ">
                <button onClick={()=>{setEditShow(true); setToolName(element?.code)  }} className="inline-flex items-center justify-center bg-sky-100 border !border-sky-200 hover:!bg-sky-200 rounded-md py-1.5 flex-23">
            ...
            {/* {editShow && <FormEdit name={category.name} closeModel={closeModel}  /> } */}
                <Link href={`/admin/service/codes/show/${element.id}`} className="inline-flex items-center justify-center bg-blue-100 border !border-blue-200 hover:!bg-blue-200 rounded-md flex-23">
...
{editShow && <FormEdit name={toolName} closeModel={closeModel}  /> }
