...
import { Category, Service, ServiceCode, Tag, Tool} from '@prisma/client';
import Image from 'next/image';
import { MdAssignmentAdd } from "react-icons/md";
import { formSchema } from '../utils/FormSchema';
import { addingCode } from '../_actions/Actions';
...
const ToolCreate = () => {
    const [code, setCode] = useState<ServiceCode| null>(null); // Use Category type
  ...
  const handleIconChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const saveUser: SubmitHandler<inputType> = async (data)=>{
      ...
      try {
        const codeData =  await addingCode(formData);
        setCode(codeData);
      } catch (error) {
        ...
        {code &&
          ...
          <div className="px-2 py-2.5 w-full flex items-center rounded-md bg-[#7648e5] mb-3">
            ...
            <div className=" flex flex-col z-0 w-full mt-2 mb-5 group">
                    <label htmlFor="category_name" className="font-medium mb-3 text-sm  text-gray-500 dark:text-gray-400 duration-300 ">Tag Name</label>
                    <div className="flex items-center w-full">
                        <div className="relative flex w-full">
                        <input {...register('code')}  type="text" name="code" id="name" className="block pl-2 h-10 px-0 z-0 w-full text-sm text-gray-900 bg-transparent border rounded-xl border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-orange-500 peer" placeholder="Web Designing ..." required />
                        </div>
                    </div> 
