...
import { getServiceWPhasesById, getServiceWWorksById } from '../../_serviceActions/ServiceActions';
import { createServicePhase, removeServicePhase, removeServiceWork } from '@/app/[locale]/admin/common/_actions/Actions';
...
const PhaseCreate = ({serviceId}:Props) => {
  ...
  const handleIconChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const saveUser: SubmitHandler<inputType> = async (data)=>{
      ...
      try {
        setLoading(true);
        const categ =  await createServicePhase(formData,serviceId);
    ...
    const removePhase  = async () => {
      ...
      const getPhaseNames = async ()=> {
        try {
            setLoading(true);
            const names = await getServiceWPhasesById(serviceId);
            setLoading(false);
   ...
   <div className="w-full sm:w-11.8/12 m-auto relative p-4 bg-white rounded-md">
        ...
        <div className=" mb-3 border border-gray-200 rounded-md">
            <div className="px-2 border-b flex gap-2 items-center border-b-gray-200 py-1.5 ">
                ...
                <span className="text-gray-600 font-medium text-md">Tools added {serviceId} </span>
            </div>
