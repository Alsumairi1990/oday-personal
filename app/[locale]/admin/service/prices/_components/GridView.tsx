import { Category, Phase, Price } from '@prisma/client';
...
import { BiBlanket, BiDiamond, BiExtension } from 'react-icons/bi';
import { BsBrowserChrome } from 'react-icons/bs';
interface Props{
    prices:PriceWithModels[],
    getSelected :(value: string) => void
    unSelected :(value: string) => void

}
const GridView = ({prices,getSelected,unSelected}:Props) => {
  ...
  return (
    ...
    {/* {
     ...
     prices && prices.map((element:any) => (
        <div className=" relative border flex my-4 w-[98%] mx-auto items-center  border-gray-200 rounded-md " >
            {!prices && loading && <div className=' w-full h-full z-40 bg-[#00000012] absolute top-0 left-0  flex items-center justify-center' style={{backdropFilter: 'blur(2px)'}}><div className='loader-2 w-4'></div></div>}
       ...
       <span className="text-red-400 text-md">{error}</span>
                ...
                <label className="relative bg-whit justify-center flex items-center  rounded-full cursor-pointer" htmlFor="checkbox">
                    <input type="checkbox"
                        ...
                        onChange={(e) => {
                            ...
                            getSelected(element?.id);
                            } else {
                               
                            unSelected(element?.id);
                            }
                        }}
                        />
                    <span
           ...
           <PriceCard  key={element.id} price={element} />
           </div>
            <div className="flex gap-x-2 flex-20  justify-center  ">
                <button onClick={()=>{setEditShow(true); setCatName(element?.name)  }} className="inline-flex items-center justify-center bg-sky-100 border !border-sky-200 hover:!bg-sky-200 rounded-md py-1.5 flex-23">
                ...
                <Link href={`/admin/category/show/${element.id}`} className="inline-flex items-center justify-center bg-blue-100 border !border-blue-200 hover:!bg-blue-200 rounded-md flex-23">
                ...
                <button 
                      type='button'
                      onClick={()=>{addSelected(element.id)}}
...
{Object.keys(groupedPrices).map((location) => (
        ...
        <div key={location} className='border p-2 border-gray-200 shadow-md rounded-md'>
          {/* <h3>{location}</h3> */}
            ...
            <div key={price.id}>
                <div className="flex items-center p-1 pb-2 border-b border-b-gray-100 bg-ugray-100">
                    <div className="">
                        ...
                        <p className="text-sm text-gray-500"><span className="text-gray-700">{service}</span> Price Details in {location}</p>
                    </div>
                     {price.image ? (
             ...
             <div className="space-y-2 mt-2 pt-2">
                <div className="flex flex-wrap gap-x-2 ">
                     {/* <p>Amount: {price.amount && price.amount.toString()}</p> */}
                ...
                <div className="flex gap-x-2 flex-100 my-7  justify-center  ">
                        ...
                        <Link href={`/admin/category/show/${location}`} className="inline-flex items-center justify-center bg-blue-100 border !border-blue-200 hover:!bg-blue-200 rounded-md flex-23">
                            <svg className="w-4 h-4 fill-blue-500"  viewBox="0 0 48 48" >
                            <path d="M0 0h48v48H0z" fill="none"/>
                            <g id="Shopicon">
                                <path d="M24,38c12,0,20-14,20-14s-8-14-20-14S4,24,4,24S12,38,24,38z M24,14c7.072,0,12.741,6.584,15.201,9.992
              ...
              {/* <p className='leading-6 line-clamp-2'>Description: {price.description}</p> */}
             </div>
              
              
              {/* <p>Effective Date: {new Date(price.effectiveDate && price.effectiveDate.toString()).toLocaleDateString()}</p>
              <p>Expiry Date: {new Date(price.expiryDate).toLocaleDateString()}</p> */}
