...
import { createExplore } from '../_actions/Actions';

type inputType = z.infer<typeof ExploresSchema>;
...
const CreateExplore = () => {
    ...
    resolver: zodResolver(ExploresSchema),
  });
  const rows = 4;
  const cols = 4;
  ...
  const handleFileChange = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    const saveUser: SubmitHandler<inputType> = async (data)=>{
      ...
      try {
        setLoading(true);
        const codeData =  await createExplore(formData);
