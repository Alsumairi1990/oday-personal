...
import { deleteMenuElement } from '@/app/[locale]/admin/setting/left-nav/_actions/Action';
import { deleteTeamById } from '../_actions/Actions';
...
const DeleteTeam = ({ids,closeModel}:Props) => {
    const [result, setResult] = useState<TeamWithModels[]>([]);
  ...
  const deleteRows = async () => {
    try {
        setLoading(true);
        const results = await deleteTeamById(ids);
         setResult(results);
         setLoading(false);
    } catch (error:any) {
        setLoading(false);
