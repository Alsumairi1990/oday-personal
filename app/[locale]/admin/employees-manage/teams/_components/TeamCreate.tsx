...
import { getLocationById, getLocations } from '@/app/[locale]/admin/location/_actions/Actions';
import AddMember from './AddMember';
type inputType = z.infer<typeof TeamSchema>;
const TeamCreate = () => {
    const [element, setElement ]= useState<TeamWithModels>(); 
    ...
    const saveUser: SubmitHandler<inputType> = async (data)=>{
      ...
      try {
        ...
        if(selectedMenuElements){
          const result =  await createTeam(formData,selectedMenuElements);
          setElement(result)
        }
  ...
  return (
   <div className="w-full sm:w-11.8/12 m-auto relative p-4 bg-white rounded-md border border-gray-200">
        ...
        {element && <div className="py-3 my-1 flex items-center">
            <LuAlertOctagon className='text-gray-500 mr-2 text-xl' />
            <span className="text-green-400 text-md">Team has beed added : with nme {element.name}</span>
            </div>
        }
