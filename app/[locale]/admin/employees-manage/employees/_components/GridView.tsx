import { Category, Testimonial, Tool, User } from '@prisma/client';
import React, { useEffect } from 'react';
import {useState} from 'react';
import Link from 'next/link';
import UserCard from '../_utils/UserCard';
...
const GridView = ({elements,getSelected,unSelected}:Props) => {
  ...
  return (
    <div>
    {
     elements.map((element:any) => (
        <div className=" relative border flex flex-wrap my-4 w-11.8/12 mx-auto items-center  border-gray-200 rounded-md " >
           ...
           <div className="flex-100 sm:flex-80">
           <UserCard  user={element} />
           </div>
            <div className="flex flex-100 sm:flex-20 gap-x-2  justify-center  ">
                {/* <button onClick={()=>{setEditShow(true); setToolName(element?.name)  }} className="inline-flex items-center justify-center bg-sky-100 border !border-sky-200 hover:!bg-sky-200 rounded-md py-1.5 flex-23">
            ...
            {/* {editShow && <FormEdit name={category.name} closeModel={closeModel}  /> } */}
                <Link href={`/admin/employees-manage/employees/show/${element.id}`} className="inline-flex items-center justify-center bg-blue-100 border !border-blue-200 hover:!bg-blue-200 rounded-md flex-23">
...
{/* {showDelete && <DeleteTestimonial ids={selectedValues} closeModel={closeDelet}  /> } */}
{}
