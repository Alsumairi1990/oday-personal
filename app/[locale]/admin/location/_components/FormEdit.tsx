...
import { Location, ServiceCode, Tool } from '@prisma/client';
...
import { editLocation, getLocationByName } from '../_actions/Actions';
import { editCode } from '../../service/codes/_actions/Actions';
...
const FormEdit = ({ name, closeModel }: FormEditProps) => {
  ...
  const saveUser: SubmitHandler<inputType> = async (data) => {
    ...
    console.log("name at form"+data.country);
    ...
    try {
      if (location?.id !== undefined) {
        await editLocation(formData, location?.id);
      }              
    } catch (error) {
  ...
  const getCategory = async () => {
    setLoading(true);
    const namedata = await getLocationByName(name);
    setLocation(namedata);
    setLoading(false);
  };
  ...
  useEffect(() => {
    ...
    if(location?.image){setImageSrc(location?.image);}
   ...
   <div className="fixed bg-[#0000003f] h-full flex overflow-hidden items-center justify-center w-full left-0 top-0 m-auto z-50 p-6 max-h[100vh]">
        ...
        >
          <div className="loader-2 w-4"></div>
           ...
           {location && <form onSubmit={handleSubmit(saveUser)} className='w-6/12 bg-white rounded-md shadow-2xl scr-container overflow-y-auto h-[95vh]' >
               ...
               <div className="p-5">
                <div className=" flex flex-col z-0 w-full mb-5 group">
                            <label htmlFor="country" className="font-medium mb-3 text-sm  text-gray-500 dark:text-gray-400 duration-300 ">Category Name</label>
                            <div className="flex items-center w-full">
                                <div className="relative flex w-full">
                                <input {...register('country')} type="text" name="country" id="name" className="block pl-2 h-10 px-0 z-0 w-full text-sm text-gray-900 bg-transparent border rounded-xl border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-orange-500 peer" placeholder="Web Designing ..." required />
                                </div>
                            </div> 
                    ...
                    <div className=" flex flex-col z-0 w-full mb-5 group">
                            <label htmlFor="city" className="font-medium mb-3 text-sm  text-gray-500 dark:text-gray-400 duration-300 ">Categoty Description</label>
                            <div className="flex items-center w-full">
                                <div className="relative flex w-full">
                                <input {...register('city')}   type="text" name="description" id="description" className="block pl-2 h-10 px-0 z-0 w-full text-sm text-gray-900 bg-transparent border rounded-xl border-gray-300 appearance-none dark:text-white dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-orange-500 peer" placeholder="Web Designing ..." required />
                                </div>
                            </div> 
