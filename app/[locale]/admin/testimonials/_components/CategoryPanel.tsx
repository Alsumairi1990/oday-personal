...
const CategoryPanel = ({ testimonial, colseModel }: FormEditProps) => {
  ...
  const addService = async () => {
    try {
        ...
        const result = await addTestimonialCategory(Number(testimonial.id), numberArray);
  ...
  const removeElem = async ()=> {
    try {
        ...
        const result = await removeTesimonialCategory(testimonial.id,removedTool);
              ...
              <div>
                <div className="py-2 flex items-center border-b border-b-gray-300 flex-wrap overflow-y-auto">
                  <div className="flex items-center rounded-md w-full gap-2 max-h-16 flex-wrap overflow-y-auto">
                   {testimonialData.categories && testimonialData.categories.length>0 ?  (
                          <div className="py-2 flex gap-2 items-center ">
                          {testimonialData.categories.map(element => (
                            <div className="flex items-center  rounded-md pl-2 pr-1 bg-white border border-gray-200">
                                ...
                                <button
                                  onClick={() => {
                                    if(testimonialData.categories.length>0) setRemovedTool(element.category.name);
                    ...
                    >
                      ...
                      {selectedMenuElements.length>0 ? (
                        ...
                        <div className="text-md inline-flex text-gray-600 font-medium capitalize">
                          <span className="px-1">Adding category</span>
                        </div>
                      )}
