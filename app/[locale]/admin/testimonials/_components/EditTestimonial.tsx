...
const EditTestimonial = ({ id}: FormEditProps) => {
  const [testimonial, setTestimonial] = useState<TestimonialWithModels | null>(null); 
  ...
  const getTestimoinal = async () => {
    try {
        ...
        const data = await getTestimonialWModelsById(serviceId);
        setTestimonial(data);
  ...
  const add = async (dataForm:FormData) => {
    try {
     const data = await editTestimonialBasicData(Number(id),dataForm);
  ...
  const editImage = async (data:FormData) => {
     ...
     try {
          const data = await editTestimonialImage(Number(id),formData);
          if(data) setTestimonial(data);
          setError('');
          setImageEdit(false);
      } catch (error:any) {
...
const editContent = async () => {
  try {
    ...
    const data = await editTestimonialContent(Number(id),content);
    setTestimonial(data);
    setError('')
  } catch (error:any) {
                ...
                <div className="flex items-center flex-col">
                    ...
                    {loading && <div className=' w-full h-full z-50 bg-[#00000012] absolute top-0 left-0  flex items-center justify-center' style={{backdropFilter: 'blur(2px)'}}><div className='loader-2 w-4'></div></div>}
                      ...
                      <div className="p2 bg-black relative rounded-md border-[.3rem] border-white">
                            <img className=' h-full opacity-70  rounded-md' src={`${baseUrl}/${testimonial?.image}`} alt="" />
                      ...
                      <div className="col-span-2 relative col-start-2 pl-5">
                         <div className="p-2 border-b flex items-center border-b-gray-200 pb-2">
                           ...
                           <form onSubmit={handleSubmit} className=" px-2 pt-1.5 rounded-md justify-center">
                                <div className="p-1.5 capitalize text-md flex items-center rounded-md">
                                 ...
                                 <div className="flex flex-70 pl-4 items-center">
                                  ...
                                  onChange={(e) => { setTestimonial({ ...testimonial, title: e.target.value }); setTouched(true)}}
                                  />
                                 </div>
                               </div>
                               ...
                               <div className="p-1.5 capitalize text-md flex items-center rounded-md">
                                 ...
                                 <div className="flex flex-70 pl-4 items-center">
                                  ...
                                  onChange={(e) =>{ setTestimonial({ ...testimonial, rating: Number(e.target.value) }); setTouched(true)}}
                                  />
                                 </div>
                               </div>
                               ...
                               <div className="p-1.5 capitalize text-md flex items-center rounded-md">
                                 <div className="flex-20 flex items-center border py-1 rounded-md pl-1">
                                   ...
                                   <span className='text-gray-800 font-medium mr-1'>Published </span> 
                                   <span className='text-gray-600 text-bxs mx-1 ml-auto'>|</span> 
                                 </div>
                                 <div className="flex flex-70 gap-x-3 pl-4 items-center">
                                    ...
                                    <div className="p-1"><span className="text-gray-400 pl-3">{testimonial.published ? 'Yes' : 'No'}</span></div>
                               ...
                               <div className="p-1.5 capitalize text-md flex items-center rounded-md">
                                 <div className="flex-20 flex items-center border py-1 rounded-md pl-1">
                                   <span className='w-6 h-6 mr-2 items-center  justify-center rounded-md  inline-flex' ><FcApprove className='text-3xl '/></span>
                                   <span className='text-gray-800 font-medium mr-1'>Verified </span> 
                                   <span className='text-gray-600 text-bxs mx-1 ml-auto'>|</span> 
                                 </div>
                                 <div className="flex flex-70 gap-x-3 pl-4 items-center">
                                    ...
                                    <div className="flex items-center bg-gray-50 px-2 border border-gray-200 rounded-md dark:border-gray-700">
                                        <input id="bordered-radio-1"
                                         onChange={()=> setTouched(true)}
                                         type="radio" value="yes" name="verified" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                                        <label htmlFor="bordered-radio-1" className="w-full py-1.5 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">Yes</label>
                                    </div>
                                    <div className="flex items-center bg-gray-50 px-2 border border-gray-200 rounded-md dark:border-gray-700">
                                        <input 
                                        onChange={()=> setTouched(true)}
                                         id="bordered-radio-2" type="radio" value="no" name="verified" className="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600" />
                                        <label htmlFor="bordered-radio-2" className="w-full py-1.5 ms-2 text-sm font-medium text-gray-900 dark:text-gray-300">No</label>
                                    </div>
                                    <div className="p-1"><span className="text-gray-400 pl-3">{testimonial.verified ? 'Yes' : 'No'}</span></div>
                                 </div>
                               </div>
                               <button
                               type='submit'
                                className={`ml-auto flex items-center absolute right-2 bottom-1 px-1.5 py-0.5 rounded-md ${touched === true ? 'bg-gray-800' : 'bg-gray-500'}`}
                    ...
                    <div className="p-2 w-full relative rounded-md my-6 bg-white border border-gray-200">
                        ...
                        <div className="px-2">
                          <div className="flex border-b border-b-gray-300 pt-1 pb-2">
                            <div className="flex gap-x-2 items-center">
                              ...
                              <span className="text-gray-700 text-md font-semibold">Testimonial Content</span>
                            </div>
                            <button
                              className='ml-auto'
                      ...
                      <div className="px-2 w-full">
                            <div className="flex  pt-1 pb-2">
                              <div className="flex gap-x-2 items-center">
                                <span className=""><MdOutlineContentPasteGo className='text-gray-700 text-xl' /> </span>
                                <span className="text-gray-700 text-md font-semibold">Edit Testimonial Content</span>
                   ...
                   <div className="w-full">
                      <CategoryPanel testimonial={testimonial} colseModel={closeCategory}  />
                    </div> 

                    <div className="w-full">
                     <TagPanel testimonial={testimonial} colseModel={closeCategory}  />
                    </div> 
