...
import { addProductMedia, getProductImagesById } from '../../products/_actions/Actions';
interface Props {
    productId : string,
  }
const AddMedia = ({productId}:Props) => {
  ...
  const getWorkImages = async ()=> {
    try {
        setLoading(true);
        const urls = await getProductImagesById(productId);
        setLoading(false);
        setImages(urls);
        setError(null); 
  ...
  const addMed1 = async (e:FormData)=>{
    try {
      alert('called')
      setLoading(true);
   const success = await addProductMedia(e,productId);
   alert('called')
    setLoading(false);
