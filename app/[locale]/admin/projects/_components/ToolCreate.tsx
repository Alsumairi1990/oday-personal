...
import { addProjectTool, createProjectTool, getProjectToolslById,  getTools, removeProjectTool } from '../_actions/Actions';
...
const getSelected= (selected:string)=>{
   ...
   const addTool = async () => {
    try {
      ...
      setLoading(true);
      const names = await addProjectTool(projectId, numberArray);
      setLoading(false);
      setToolNames(names);
      setError(null);
    } catch (error: any) {
    ...
    const createCategory = async () => {
      try {
        if(projectId != '' && toolName != ''){
          setLoading(true);
          const catname = await createProjectTool(projectId , toolName);
          setLoading(false);
          setError(null)
