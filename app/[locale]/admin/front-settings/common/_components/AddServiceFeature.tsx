...
import { CreateServiceFeature } from '../_actions/Actions';

type inputType = z.infer<typeof ServiceFeaturesSchema>;
...
const AddServiceFeature = () => {
    ...
    resolver: zodResolver(ServiceFeaturesSchema),
  });
  const rows = 4;
  const cols = 4;
  ...
  const unPaymentSelect = (value:string) => {
    ...
    const saveUser: SubmitHandler<inputType> = async (data)=>{
      ...
      try {
        setLoading(true);
        const codeData =  await CreateServiceFeature(formData);
