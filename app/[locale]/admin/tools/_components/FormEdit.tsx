...
import { editTool, getToolByName} from '../_actions/Actions';
...
const FormEdit = ({ name, closeModel }: FormEditProps) => {
  ...
  const saveUser: SubmitHandler<inputType> = async (data) => {
    ...
    console.log("name at form"+data.name);
    ...
    try {
      if (tool?.id !== undefined) {
        await editTool(formData, tool?.id);
      }              
    } catch (error) {
  ...
  const getCategory = async () => {
    const tagName = await getToolByName(name);
    setTool(tagName);
  };
  ...
  return (
   <div className="fixed bg-[#0000003f] h-full flex overflow-hidden items-center justify-center w-full left-0 top-0 m-auto z-50 p-6 max-h[100vh]">
           {tool && <form onSubmit={handleSubmit(saveUser)} className='w-6/12 bg-white rounded-md shadow-2xl scr-container overflow-y-auto h-[95vh]' >
