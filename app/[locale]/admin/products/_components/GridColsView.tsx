...
import CardColCard from '../_utils/GridColCard';
interface Props{
...
const GridColsView = ({products,getSelected,unSelected}:Props) => {
  ...
  return (
    <div className='grid grid-cols-3 gap-4 w-11.7/12 mx-auto'>
    {
     products.map((element:ProductWithModels) => (
        <div className=" relative border flex flex-col my-4 w-[98%] mx-auto items-center  border-gray-200 rounded-md " >
           ...
           <div className="">
           <CardColCard  key={element.id} product={element} />
           </div>
           <div className="flex gap-x-2 my-2 justify-center px-3 ">
                <button onClick={()=>{setEditShow(true); setCatName(element?.name)  }} className="inline-flex items-center justify-center bg-sky-100 border !border-sky-200 hover:!bg-sky-200 px-1.5 rounded-md py-1.5 flex-30">
            ...
            {/* {editShow && <FormEdit name={element.name} closeModel={closeModel}  /> } */}
                <Link href={`/admin/element/show/${element.id}`} className="inline-flex items-center justify-center bg-blue-100 border !border-blue-200 hover:!bg-blue-200 px-1.5 rounded-md flex-30">
...
{/* {showDelete && <Deleteelement elementIds={selectedValues} closeModel={closeDelet}  /> } */}
